...
class PbbExport implements FromQuery, WithHeadings, WithEvents, WithColumnFormatting, WithColumnWidths
{
    use Exportable;

    function __construct($id, $waktu)
    {
        $this->id = $id;
        $this->waktu = $waktu;
    }
    ...
    {
        return PBB::select('instansi', 'kelurahan', PBB::raw("DATE_FORMAT(db_realisasi_pbb.waktu, '%b-%Y') as waktu"), 'target_pbb', 'realisasi_bln_lalu', 'realisasi_bln_ini', 'jmlh_realisasi', 'sisa_target', 'keterangan')
            ->where('id_instansi', '=', $this->id)->where('waktu', '=', $this->waktu)->orderBy('id', 'DESC');
